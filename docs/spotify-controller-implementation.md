...
#### 踏んだ罠: サインアウトして入り直しても scope が増えないことがある

配信直後、**再ログイン済みなのに追加・削除の両方が 403 `Forbidden`** になった。
原因は 2 つ重なっていた。

1. **Spotify は既に承認済みのアプリだと同意画面を飛ばし、以前承認した範囲の
   トークンを返すことがある。** `show_dialog=true` を付けて必ず承認を通さないと、
   追加した scope が入らない。`reauthorize()` は常に付けるようにした。
2. **控えていたのが「要求した scope」だったので、手元では気づけなかった。**
   `needsReauthorization` が false のままなのでバナーも出ず、
   結果として「原因の分からない 403」になっていた。トークンに実際に付いた
   scope（token レスポンスの `scope`）を控え、リフレッシュのたびに突き合わせる。

`/playlists/*` の 403 は **素の `Forbidden` しか返ってこない**（player 系のように
`Insufficient client scope` とは言ってくれない）ので、文面での切り分けもできない。
書き込みの scope が欠けていると控えで分かっているときだけ再連携として扱い、
そもそも書き込みを試す前に足切りする。

さらに、**控えが「実測」か「当て推量」か**を持つようにした。OAuth では要求と同じ
scope なら token レスポンスの `scope` を省略してよいので、実測が取れないことがある。
その場合は要求どおり出たと仮定して控えるしかないが、仮定のまま 403 を食らったら
**403 のほうが実測**なので控えを落として再連携へ倒す。実測の控えで 403 なら scope の
話ではないので、再連携を勧めずに「権限は付いている」と言い切る。どちらの状態かは
☰ の行（`AuthService.scopeSummary`）に出す。

**それでも Spotify が新しい scope を出さないときの最後の手**は、
...
- **Spotify 製**（Discover Weekly / Daily Mix / Release Radar / Blend）— 所有者が
  `spotify` なので、id が取れていなくても落とせる
- **他人のプレイリスト**（フォロー中のもの）— 自分の id と突き合わせないと分からない

**非公開かどうかは無関係。** 自分の非公開リストは `playlist-modify-private` で普通に
書ける。文面でそこへ誘導してはいけない。

`GET /me` の id が要るのは前者ではなく後者のためで、そこに `user-read-private` が
必要（無いと `/me` が 403 になり、他人のリストを候補に出してしまう）。
それでも取りこぼしはあるので、**書き込みが 403 になったリストは覚えて候補から外す**
（`PlayerController._markReadOnly`）。API には「このリストは書き換えられるか」を
返す口が無いので、拒否されたという実測が唯一の確かな材料になる。
