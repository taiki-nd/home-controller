...
| 12 | ~~ギャップレス（方式 B）が要るかの判断（§5.3）~~ → 次の 1 曲を `SetNextAVTransportURI` で預ける形にしたので、方式 B は要らなくなった | `app/lib/services/wiim_upnp.dart` | ✅ |
| 17 | **実機で「別のアプリを開いたまま鳴り続けるか」を確かめる**（§5.3、issue #8）。画面を消した状態・電話で中断された後も見る | iPad | |
...
UPnP の PlayQueue を使えば WiiM 側にキューを丸ごと持たせられるが、
**実機なしでは検証できない**（DIDL の組み方が当たっているか確かめようがない）
ので着手していない。方式 B を検討するときに一緒に測る。

#### 次の 1 曲だけは先に預ける（issue #8）

方式 A の弱点は **見ている人が要る**こと。前面から外れるとポーリングが止まり、
曲が終わっても誰も次を投げない——別のアプリを開いた瞬間にキューがそこで止まる。

そこで、鳴らし始めた時点で **次の 1 曲を `SetNextAVTransportURI` で本体に
預ける**。標準の AVTransport アクションで、実機（WiiM Ultra /
Linkplay.5.2.826052）の `rendertransportSCPD.xml` にも宣言がある。

- 引数名は `NextURI` / `NextURIMetaData`。**`SetAVTransportURI` の
  `CurrentURI` とは綴りが違う**
- 続けて `Play` は送らない。**予約するだけ**——送ると今の曲が頭から鳴り直す
- キューの最後まで来たら**必ず空で上書きする**。残っていると、止まるはずの
  ところで 1 曲余計に鳴る
- 曲間のギャップも同時に消える（本体が続けて掴むため）

前面に戻ったときの辻褄合わせは `QobuzController._absorbAutoAdvance`。本体が
返す曲名（DIDL で渡した `dc:title` がそのまま返る）が預けた曲のものになって
いたら、**送り直さずに `_index` だけ進めて**、さらに次を預ける。同じ曲を 2 回
続けて積んだときは曲名で区別が付かないので、「終端まで行って頭に戻った」動きで
見る。

**預けられるのは 1 曲ぶんだけ。** これ単体だと、バックグラウンドのまま 2 曲ぶん
流れ切ったところで止まる。そこから先は次の無音キープアライブが受け持つ。

#### 無音を鳴らしてアプリを生かす（issue #8）

先読みで足りないのは、**アプリが生きていないと次の 1 曲を預け直せない**から。
iOS はバックグラウンドのアプリを数十秒で suspend し、そうなると Dart の
`Timer` も止まる。

生き続けるには `UIBackgroundModes` が要るが、**LAN の機器を見張るための口は
無い。**

| モード | 使えるか |
|---|---|
| `fetch` (BGAppRefreshTask) | 起こされるのは数十分〜数時間おき。曲の切れ目に間に合わない |
| `processing` | 充電中かつ端末アイドルが前提 |
| `location` | 常時位置情報の許可が要る。実態と合わない |
| `voip` | PushKit 前提 |
| **`audio`** | **これだけが実用的** |

`audio` は「アプリ自身が音を出していること」が条件なので、**無音を鳴らし
続ける**（`app/ios/Runner/PlaybackKeepAlive.swift`）。鳴っているのは WiiM で、
端末は音を出していない——それでも端末側が生きていないとキューは進まない。

- `.playback` + `.mixWithOthers`。**他のアプリの音を止めない**（ダッキングも
  中断も起こさない）
- `numberOfLoops = -1` で切れ目なく回す。**一瞬でも止めると suspend される**
- 中断（電話など）が終わったらかけ直す。戻さないとそこで止まる
- 無音の WAV はアセットではなく Swift でその場で組む。数十バイトのヘッダを
  足すためだけに project.pbxproj を触りたくない
- **掴む条件は「鳴っていること」**。停止中に音声セッションを掴んだままにしない
  （`QobuzController.setForeground`）。キューが尽きたらその場で手放す

背面ではポーリング間隔を `pollWhileBackground`（10 秒）まで落とす。誰も見て
いないのでシークバーを動かす必要が無く、やることは「本体がどこまで進んだか見て、
次の 1 曲を預け直す」だけ。

**先読みと二段構え。** キープアライブが落ちても（音声セッションを取られた、
iOS の気まぐれ）、預けてある 1 曲ぶんは繋がる。

**公開ビルドにはこの宣言を残さない。** `ENABLE_MUSIC=false` のビルドには
QOBUZ 自体が入らないので、音を出さないアプリが `audio` を宣言しているだけに
なり審査で刺さる。CI が Info.plist から抜く（`docs/release-strategy.md` §3）。

無制限に鳴らし続けるだけなら PlayQueue1 にキューごと渡す手もあるが、それは
上のとおり別の話（キューの持ち主が変わる）。
