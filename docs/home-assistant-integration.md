...
| 対策 | 内容 |
|---|---|
| **無操作で music に自動復帰** | 3 分。**これが本命。** home は「用があるときだけ開く画面」と割り切る |
| ☰ は行を増やさず左上に置く | music はデバイスピルの左（**既存の行に同居させ、列を増やさない**）、home はレール見出しの行。輝度を落として面の差を小さくする。同じ絵が何時間も残らないのは上の自動復帰が担保する |

> ☰ を出すとデバイスピルがそのぶん右へずれる。**ピルの点の x はデバイス一覧の
> ポップオーバーの点と縦に揃える必要がある**ので、`devicePillDotXFor(hasMenu:)`
> で両方が同じ値を見るようにしてある。`MenuButton` の幅を変えたらここも動く。
